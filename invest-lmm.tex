...
\hlstd{params} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlkwd{fixef}\hlstd{(transformed.lme),} \hlnum{500}\hlstd{)}
\hlstd{covmat} \hlkwb{<-} \hlkwd{diag}\hlstd{(}\hlnum{3}\hlstd{)}  \hlcom{# set up var/cov matrix}
\hlstd{covmat[}\hlnum{1}\hlopt{:}\hlnum{2}\hlstd{,} \hlnum{1}\hlopt{:}\hlnum{2}\hlstd{]} \hlkwb{<-} \hlkwd{vcov}\hlstd{(transformed.lme)}  \hlcom{# fixed-effects var/cov matrix}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}xyplot"{}}}\end{kframe}
\end{knitrout}
...
\end{kframe}
\end{knitrout}
For example, to estimate $\sigma_0^2 = \widehat{\VAR}\left[\mathcal{Y}_0\right]$, we have \texttt{varY(fit.lme4, x = x0.est)}, which gives \ensuremath{1.757\times 10^{4}}, the same value used in the previous section.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{pb} \hlkwb{<-} \hlkwd{bootMer}\hlstd{(fit.lme4,} \hlkwc{nsim} \hlstd{=} \hlnum{9}\hlstd{,} \hlkwc{seed} \hlstd{=} \hlnum{105}\hlstd{,} \hlkwc{FUN} \hlstd{=} \hlkwa{function}\hlstd{(}\hlkwc{.}\hlstd{) \{}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{QI.boot} \hlkwb{<-} \hlstd{pb}\hlopt{$}\hlstd{t[,} \hlnum{3}\hlstd{]}  \hlcom{# bootsrap replicates of Q_I}
\hlstd{qvals} \hlkwb{<-} \hlkwd{quantile}\hlstd{(QI.boot,} \hlkwd{c}\hlstd{(}\hlnum{0.025}\hlstd{,} \hlnum{0.975}\hlstd{))}  \hlcom{# sample quantiles}
\hlkwd{invest}\hlstd{(transformed.lme,} \hlkwc{y0} \hlstd{=} \hlnum{500}\hlstd{,} \hlkwc{q1} \hlstd{= qvals[}\hlnum{1}\hlstd{],} \hlkwc{q2} \hlstd{= qvals[}\hlnum{2}\hlstd{])}
\end{alltt}
\begin{verbatim}
## estimate    lower    upper 
##     8.02     4.62     9.30
...
\begin{tabular}{lcccc}
  ...
  $CI_{percentile}\boot\left(x_0\right)$     & 8.016 & 1.369 & (6.996, 11.143) & 4.147 \\
  $CI_{wald}\boot\left(x_0\right)$           & 8.016 & 1.369 & (4.099, 9.363) & 5.264 \\
  $CI_{inv}\boot\left(x_0\right)$      & 8.016 & 1.369 & (4.623, 9.303) & 4.68 \\
