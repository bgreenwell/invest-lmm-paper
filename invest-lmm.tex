...
\end{kframe}
\end{knitrout}
Theoretically, the parameter estimates from this model should be the same as those from \texttt{fit.nlme}; however, there are likely to be small numerical differences between the two.  For this reason, let us re-estimate $\widehat{x}_0$ using \texttt{fit.lme4}.  Since \texttt{invest} does not work on objects of class \texttt{lmer}, we have to do things manually:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\hlkwd{invest}\hlstd{(fit.nlme,} \hlkwc{y0} \hlstd{=} \hlnum{500}\hlstd{,} \hlkwc{q1} \hlstd{= qvals[}\hlnum{1}\hlstd{],} \hlkwc{q2} \hlstd{= qvals[}\hlnum{2}\hlstd{])}
\end{alltt}
\begin{verbatim}
## estimate    lower    upper 
##     8.02     4.62     9.30
