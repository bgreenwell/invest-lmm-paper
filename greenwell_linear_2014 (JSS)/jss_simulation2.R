...
## Random intercept and slope model (nlme version)
bladder_nlme <- lme(HD^(3/2) ~ volume, random = list(subject = pdDiag(~volume)), 
                    data = bladder) 

## Variance function (lme4 version)
...
## Fit LMM to simulated data
fitModel <- function(.data, LMM = TRUE) {
  if (LMM) {  # random intercept and slope model
    lme(HD ~ volume, random = list(subject = pdDiag(~volume)), 
        data = .data, control = lmeControl(opt = "optim"))
  } else {  # simple linear regression
    lm(HD ~ volume, data = .data)
  }
}

## Inverse estimation function
getInterval <- function(model, y0 = 500, mean.response = FALSE) {
  Y0 <- if (mean.response) y0 else y0 + rnorm(1, mean = 0, sd = sd_y0)
  w <- invest(model, y0 = Y0, interval = "Wald", 
              mean.response = mean.response, lower = -5, upper = 40,
              tol= 1e-10)
  i <- invest(model, y0 = Y0, interval = "inversion", 
              mean.response = mean.response, lower = -5, upper = 40, 
              tol= 1e-10)
...
summarizeIntervals <- function(x, x0 = 8.015521) {
...
## Simulation function
simulate <- function(nsim = 1000, m = 23, n = 8, ...) {
  data_list <- rlply(nsim, genData(m = m, n = n))
  model_list <- llply(data_list, fitModel)
  intervals <- ldply(model_list, getInterval, ..., .parallel = TRUE)
  summarizeIntervals(intervals)  
}

# set.seed(101)
# simulate(nsim = 1000, m = 5, n = 5)
...
mn_grid <- setNames(expand.grid(m, n), c("m", "n"))
df_reg <- NULL
set.seed(101)
for (i in seq_len(nrow(mn_grid))) {
  df_reg <- rbind(df_reg, 
                  simulate(nsim = 1000, m = mn_grid[i, 1], n = mn_grid[i, 2],
                           mean.response = TRUE))
  save(df_reg, file = "/home/w108bmg/Desktop/jss-sim-reg.RData")
  print(paste(round(i/nrow(mn_grid) * 100), "% complete", sep = ""))
  
}
df_reg <- cbind(mn_grid, df_reg)


library(reshape2)
df_coverage <- melt(df_reg, id.vars = c("m", "n"), 
                    measure.vars = c("w_cp", "i_cp"),
...
for (i in seq_len(nrow(mn_grid))) {
  df_cal <- rbind(df_cal, 
                      simulate(nsim = 1000, m = mn_grid[i, 1], n = mn_grid[i, 2]))
  save(df_cal, file = "/home/w108bmg/Desktop/jss-sim-cal.RData")
  print(paste(round(i/nrow(mn_grid) * 100), "% complete", sep = ""))
  
}
df_cal <- cbind(mn_grid, df_cal)