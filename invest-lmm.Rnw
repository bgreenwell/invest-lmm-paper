...
<<>>=
library(lme4)
transformed.lme4 <- lmer(HD ^ (3 / 2) ~ volume + (0 + 1 | subject) + 
...
<<>>=
fe <- unname(fixef(transformed.lme4))  # fixed-effects
...
@
For example, to estimate $\sigma_0^2 = \widehat{\VAR}\left[\mathcal{Y}_0\right]$, we have \texttt{varY(transformed.lme4, x = x0.est)}, which gives \Sexpr{varY(transformed.lme4, x = x0.est)}, the same value used in the previous section.
...
<<eval=FALSE>>=
pboot <- bootMer(transformed.lme4, nsim = 9, seed = 105, FUN = function(.) {
...
inversion.pboot <- invest(transformed.lme, y0 = 500, q1 = qvals[1], q2 = qvals[2])
