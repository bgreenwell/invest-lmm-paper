...
<<echo=FALSE>>=
fit.lme <- lme(HD ^ (3 / 2) ~ volume, 
                       random = list(subject = pdDiag( ~ volume)),
                       data = bladder)
# knitr::kable(summary(fit.lme)$tTable, format = "latex", digits = 3)
@
...
# LMM, etc.
fit.lmer <- lmer(HD ^ (3 / 2) ~ volume + (0 + 1 | subject) + 
                           (0 + volume | subject), data = bladder)
fe <- unname(fixef(fit.lmer))  # fixed-effects
...
# Wald interval
wald <- invest(fit.lme, y0 = 500, interval = "Wald")

# Inversion interval
inversion <- invest(fit.lme, y0 = 500)
...
inversion.pboot <- 
  invest(fit.lme, y0 = 500, q1 = qvals[1], q2 = qvals[2])
...
<<simulation, echo=FALSE, fig.width=8, fig.height=4, fig.cap='Coverage probability of 95\\% confidence intervals for simulated bladder data. The coverage estimates based on the inversion method are colored blue.', fig.pos='!htb'>>=
...
<<>>=
library(investr)
invest(fit.lme, y0 = 500, interval = "none")
@
...
<<>>=
invest(fit.lme, y0 = 500, interval = "Wald")
@
...
params <- c(fixef(fit.lme), 500)
covmat <- diag(3)  # set up var/cov matrix
covmat[1:2, 1:2] <- vcov(fit.lme)  # fixed-effects var/cov matrix
...
<<>>=
invest(fit.lme, y0 = 500, interval = "inversion")
@
...
invest(fit.lme, y0 = 500, q1 = tvals[1L], q2 = tvals[2L])
@
...
@
For example, to estimate $\sigma_0^2 = \widehat{\VAR}\left[\mathcal{Y}_0\right]$, we have \texttt{varY(fit.lmer, x = x0.est)}, which gives \Sexpr{varY(fit.lmer, x = x0.est)}, the same value used in the previous section.
...
<<eval=FALSE>>=
pboot <- bootMer(fit.lmer, nsim = 9, seed = 105, FUN = function(.) {
