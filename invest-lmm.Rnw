...
@
For example, to estimate $\sigma_0^2 = \widehat{\VAR}\left[\mathcal{Y}_0\right]$, we have \texttt{varY(fit.lme4, x = x0.est)}, which gives \Sexpr{varY(fit.lme4, x = x0.est)}, the same value used in the previous section.
...
\begin{tabular}{lcccc}
  ...
  $CI_{inv}\boot\left(x_0\right)$      & \Sexpr{x0.est} & \Sexpr{se.boot} & (\Sexpr{inversion.pb$lower}, \Sexpr{inversion.pb$upper}) & \Sexpr{lengths[5]} \\
...
pb <- bootMer(fit.lme4, nsim = 99, seed = 105, FUN = function(.) {
